...
// BsvRPC is an interface to JSON-RPC bitcoind service.
type BsvRPC struct {
	*btc.BitcoinRPC
}
...
func (b *BsvRPC) GetBlock(hash string, height uint32) (*bchain.Block, error) {
	var err error
	if hash == "" && height > 0 {
...
func (b *BsvRPC) GetBlockRaw(hash string) (string, error) {
...
func (b *BsvRPC) GetBlockBytes(hash string) ([]byte, error) {
...
func (b *BsvRPC) GetBlockInfo(hash string) (*bchain.BlockInfo, error) {
...
func (b *BsvRPC) GetBlockFull(hash string) (*bchain.Block, error) {
...
func (b *BsvRPC) EstimateFee(blocks int) (big.Int, error) {
	...
	if b.ChainConfig.CoinShortcut == "BSV" {
